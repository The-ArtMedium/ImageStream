...
# LocalSuite shared database — lets AthleteTagger's matches be found
# later alongside what FocusCheck or LocalRAW already know about the
# same photos. Optional: if this file isn't present yet, AthleteTagger
# still works exactly as before, just without that memory.
try:
    import localsuite_db
    LOCALSUITE_DB_AVAILABLE = True
except ImportError:
    LOCALSUITE_DB_AVAILABLE = False
...
def load_or_compute_encodings(athlete_dict, threshold):
    ...
    cache_data = {'hash': config_hash, 'encodings': encodings, 'names': names}
...
def process_image(args):
    ...
    try:
        ...
        return (image_path, new_path, athletes_str), True
...
def scan_folder(folder_path, known_encodings, known_names, threshold, dry_run=False, max_workers=None):
    ...
    worker_args = [(path, known_encodings, known_names, threshold) for path in image_paths]
    
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        results = list(tqdm(executor.map(process_image, worker_args), total=len(image_paths), desc="Scanning photos", unit="img"))
    
    for result, has_faces in results:
        stats.total_processed += 1
        if result:
            old_path, new_path, athletes_str = result
            try:
                os.rename(old_path, new_path)
                stats.files_renamed += 1
                logger.info(f"✓ Renamed: {Path(old_path).name} → {Path(new_path).name}")

                # Update the shared LocalSuite database to follow the file
                # under its new name — done here in the main process, one
                # at a time, so multiple worker processes never fight over
                # the same database file. If FocusCheck or another tool
                # already had a row for this photo under its old name,
                # rename_photo() carries that same row forward instead of
                # starting a new one.
                if LOCALSUITE_DB_AVAILABLE:
                    try:
                        localsuite_db.rename_photo(
                            old_path, new_path,
                            athlete_tag=athletes_str.replace("_", ", "),
                        )
                    except Exception:
                        pass  # Never let a database hiccup interrupt tagging.
            except Exception as e:
                logger.error(f"❌ Failed to rename {old_path}: {e}")
                stats.errors += 1

def main():
    parser = argparse.ArgumentParser(description="ImageStream Athlete Tagger - Organize photos by detected athletes", formatter_class=argparse.RawDescriptionHelpFormatter, epilog="""
Examples:
  ...
  python athlete_tagger.py /path/to/photos --workers 4
    ...
    scan_folder(args.folder, known_encodings, known_names, threshold, dry_run=args.dry_run, max_workers=args.workers)
