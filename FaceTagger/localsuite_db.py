...
def rename_photo(old_filepath, new_filepath, **fields):
    """
    Use this instead of upsert_photo() whenever a tool renames or
    moves a file it might already know about (AthleteTagger, for
    instance, always renames). This keeps whatever an earlier tool
    already recorded about the photo — instead of losing that row
    and quietly starting a new one under the new name.

    Any extra fields (like athlete_tag=...) are set on the same call.
    """
    conn = get_connection()
    old_id = _make_id(old_filepath)

    existing = conn.execute(
        "SELECT id FROM photos WHERE id=?", (old_id,)
    ).fetchone()

    if existing:
        thumb = get_or_create_thumbnail(new_filepath)
        updates = dict(fields)
        updates["filepath"] = str(Path(new_filepath).resolve())
        updates["filename"] = Path(new_filepath).name
        if thumb:
            updates["thumbnail_path"] = thumb
        set_clause = ", ".join(f"{k}=?" for k in updates)
        conn.execute(
            f"UPDATE photos SET {set_clause} WHERE id=?",
            (*updates.values(), old_id),
        )
        conn.commit()
        conn.close()
        return old_id
    ...
    conn.close()
    # No earlier row under the old name — just record it fresh,
    # keyed on the new (final) path going forward.
    return upsert_photo(new_filepath, **fields)
