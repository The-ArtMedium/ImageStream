...
# LocalSuite shared database — lets FocusCheck's sharpness verdicts
# be queried later, alongside what AthleteTagger and LocalRAW know
# about the same photos. Optional: if this file isn't present yet,
# FocusCheck still works exactly as before, just without that memory.
try:
...
class App:
    ...
    def _scan(self):
        ...
        for f, sc, shutter, bt in file_data:
            ...
            # Write this verdict into the shared LocalSuite database,
            # so AthleteTagger, LocalRAW, or a future gallery can find
            # it later — without FocusCheck needing to know about them.
            if LOCALSUITE_DB_AVAILABLE:
                try:
                    localsuite_db.upsert_photo(
                        str(f),
                        format=f.suffix.lower().lstrip("."),
                        sharpness_verdict=cat,
                        sharpness_score=float(sc),
                    )
                except Exception:
                    pass  # Never let a database hiccup interrupt culling.
