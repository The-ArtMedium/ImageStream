...
class EditorScreen(Screen):
    ...
    def on_enter(self):
        ...
        # Add Clip button
        add_btn = Button(text="ADD CLIP", size_hint=(0.9,0.15), pos_hint={'center_x':0.5,'center_y':0.1},
                         background_color=(0.96,0.65,0.14,1), background_normal='', color=(0,0,0,1), bold=True)
        add_btn.bind(on_release=self.add_clip)
    ...
    def add_clip(self, instance):
        if self.start_time < self.end_time:
            self.clips.append((self.start_time, self.end_time))
            self.status.text = f"Clips saved: {len(self.clips)}"
            # Reset buttons
