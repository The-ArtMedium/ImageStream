...
LOCALES = {
    ...
    'es': {'select': "SELECCIONAR MAESTRO", 'load': "CARGAR PELÍCULA", 'in': "MARCAR INICIO", 'out': "MARCAR FINAL", 'save': "GUARDAR SIN PÉRDIDA", 'ready': "Listo", 'saving': "Guardando...", 'close': "FERMER LE MASTER", 'exit': "SALIR", 'err_time': "Ver IN/OUT"}
}
...
if platform == 'android':
    ...
    from kvdroid.tools import uri_tool
...
class MainScreen(Screen):
    ...
    def on_load(self, *args):
        if self.fc.selection:
            raw_path = self.fc.selection[0].replace('file://', '')
    ...
    def deferred_load(self, dt):
        try:
            if platform == 'android':
                ...
                # The Handshake: URI instead of Raw Path
                video_uri = uri_tool.get_uri(self.video_path)
                self.player = ExoPlayer()
                self.player.prepare(video_uri)
                
                # Wait for Java engine metadata
            ...
            self.status.text = f"JVM Handshake Fail: {str(e)[:40]}"
